...
if ($edit_id) {
  ...
  if(mysqli_stmt_fetch($st)){$selected_mhs=$idm;$jml=$jb;$tgl=$tb;}
...
if ($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  $jml = $_POST['jml_bayar'] ?? '';
  if ($selected_mhs<=0 || $jml==='' ) $msg='Lengkapi field';
  else {
    if (!empty($_POST['id'])) {
      $id=(int)$_POST['id'];
...
?>
<!doctype html><html><head><meta charset="utf-8"><title>Tambah/Edit Pembayaran</title>
<link rel="stylesheet" href="style.css"></head><body>
<div class="container"><h2><?= $edit_id ? "Edit Pembayaran":"Tambah Pembayaran" ?></h2>
...
<label>Jumlah Bayar</label><input type="number" step="0.01" name="jml_bayar" value="<?= htmlspecialchars($jml) ?>">
<label>Tanggal Bayar</label><input type="date" name="tgl_bayar" value="<?= htmlspecialchars($tgl ?: date('Y-m-d')) ?>">
