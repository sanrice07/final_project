...
?>
<!doctype html><html><head><meta charset="utf-8"><title>User</title><link rel="stylesheet" href="style.css"></head><body>
<div class="container"><h2>User</h2><a class="btn" href="manage_user_table.php">Tambah User</a>
<?php
if (!$res) echo "<div class='msg error'>".htmlspecialchars(mysqli_error($con))."</div>";
else {
  if (mysqli_num_rows($res)===0) echo "<div class='msg'>Belum ada user.</div>";
  else {
    echo "<table class='table'><thead><tr><th>ID</th><th>Username</th><th>Aksi</th></tr></thead><tbody>";
    while($r=mysqli_fetch_assoc($res)){
      $id=$r['id_user']; $u=htmlspecialchars($r['username']);
