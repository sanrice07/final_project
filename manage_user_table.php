  ...
  if ($username === '')
    ...
    if (!empty($_POST['id'])) {
      ...
      if ($password === '') {
        ...
        mysqli_stmt_bind_param($st, "si", $username, $id);
        mysqli_stmt_execute($st);
        mysqli_stmt_close($st);
      } else {
        $hash = password_hash($password, PASSWORD_DEFAULT);
        $st = mysqli_prepare($con, "UPDATE `user` SET username=?, password=? WHERE id_user=?");
        mysqli_stmt_bind_param($st, "ssi", $username, $hash, $id);
        mysqli_stmt_execute($st);
        mysqli_stmt_close($st);
      }
      header('Location: display_user_table.php');
      exit;
    } else {
      // insert
      if ($password === '')
        ...
        mysqli_stmt_bind_param($st, "ss", $username, $hash);
        mysqli_stmt_execute($st);
        mysqli_stmt_close($st);
        header('Location: display_user_table.php');
        exit;
...
<body>
  <div class="container">
    <h2><?= $edit_id ? "Edit User" : "Tambah User" ?></h2>
    <?php if ($msg)
    ...
    <form method="post">
      ...
      <label>Password <?= $edit_id ? "(biarkan kosong kalau tidak ingin mengganti)" : "" ?></label><input
